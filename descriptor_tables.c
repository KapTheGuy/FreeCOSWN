...
static void init_gdt()
{
	...
	gdt_set_gate(5, (u32int)&tss_entry, sizeof(tss_entry) - 1, 0xE9, 0x00); // TSS entry
 
    gdt_flush((u32int)&gdt_ptr);
}

static void init_tss() {
    // Set the stack pointer for the TSS
    tss_entry.ss0 = 0x10; // Kernel data segment
    tss_entry.esp0 = 0;   // Stack pointer to be set (typically to a predefined stack)

    // Set up the TSS descriptor in the GDT
    gdt_set_gate(5, (u32int)&tss_entry, sizeof(tss_entry) - 1, 0xE9, 0x00); // 0xE9 is the access byte for a TSS
